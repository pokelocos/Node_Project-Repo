...
public class ConnectionInformationView : MonoBehaviour
{
    ...
    public void Display(Port port, NodeView node)
    {
        ...
        node_image.sprite = node.GetNodeData().icon;
        node_image.color = node.GetNodeData().color.Darker();
        nodeBackground_image.color = node.GetNodeData().color;
