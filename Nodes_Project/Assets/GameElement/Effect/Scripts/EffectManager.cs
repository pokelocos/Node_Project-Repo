...
namespace MicroFactory.Effects
{
    class EffectManager : MonoBehaviour
    {
        ...
        {
            var view = Instantiate(pref_view, transformParent);
            view.SetInfo(effect, effect.color, Color.white);
            ...
            effect.ApplyAditiveEffect(nodes);
            view.OnEndEffect = (effect) => { effect.RemoveAditiveEffect(nodes); }; // cheaquear
        }
